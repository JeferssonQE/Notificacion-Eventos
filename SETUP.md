# 🔧 Guía de Configuración Completa

Esta guía te ayudará a configurar todas las credenciales y servicios necesarios para ejecutar DólarBot.

## 📋 Tabla de Contenidos

1. [Configuración de Gmail](#1-configuración-de-gmail)
2. [Configuración de Supabase](#2-configuración-de-supabase)
3. [Token de SUNAT API](#3-token-de-sunat-api)
4. [Variables de Entorno](#4-variables-de-entorno)
5. [GitHub Actions Secrets](#5-github-actions-secrets)

---

## 1. Configuración de Gmail

### Paso 1: Habilitar Autenticación de 2 Factores

1. Ve a tu [Cuenta de Google](https://myaccount.google.com/)
2. Navega a `Seguridad`
3. Habilita `Verificación en dos pasos`

### Paso 2: Generar App Password

1. En la misma sección de Seguridad
2. Busca `Contraseñas de aplicaciones`
3. Selecciona `Correo` y `Otro (nombre personalizado)`
4. Escribe "DolarBot" como nombre
5. Copia la contraseña de 16 caracteres generada

### Variables necesarias:
```env
EMAIL_USER=tu_email@gmail.com
EMAIL_PASS=xxxx xxxx xxxx xxxx  # App Password (sin espacios)
EMAIL_TO=destinatario@gmail.com
```

---

## 2. Configuración de Supabase

### Paso 1: Crear Proyecto

1. Ve a [supabase.com](https://supabase.com)
2. Crea una cuenta o inicia sesión
3. Click en `New Project`
4. Completa:
   - **Name**: dolar-bot
   - **Database Password**: (guarda esta contraseña)
   - **Region**: South America (São Paulo)
5. Espera 2-3 minutos mientras se crea el proyecto

### Paso 2: Obtener Credenciales

1. En el dashboard, ve a `Settings` → `API`
2. Copia:
   - **Project URL**: `https://xxxxx.supabase.co`
   - **anon/public key**: `eyJhbGc...`

3. Ve a `Settings` → `Database`
4. Copia la **Connection String** en modo `URI`

### Paso 3: Crear Tabla de Dólar

Ejecuta este SQL en el SQL Editor de Supabase:

```sql
-- Crear tabla dolar
CREATE TABLE dolar (
    id SERIAL PRIMARY KEY,
    origen VARCHAR(100) NOT NULL,
    fecha DATE NOT NULL,
    precio_compra DECIMAL(10, 4) NOT NULL,
    precio_venta DECIMAL(10, 4) NOT NULL,
    diferencia_ayer DECIMAL(10, 4),
    created_at TIMESTAMP DEFAULT NOW(),
    UNIQUE(origen, fecha)
);

-- Crear índices para mejorar performance
CREATE INDEX idx_dolar_origen ON dolar(origen);
CREATE INDEX idx_dolar_fecha ON dolar(fecha DESC);
CREATE INDEX idx_dolar_origen_fecha ON dolar(origen, fecha DESC);

-- Tabla para recordatorios (opcional)
CREATE TABLE recordatorio_dolar (
    id SERIAL PRIMARY KEY,
    numero VARCHAR(20) NOT NULL,
    valor_objetivo DECIMAL(10, 4) NOT NULL,
    porcentaje DECIMAL(5, 2) DEFAULT 0,
    movimiento VARCHAR(10) CHECK (movimiento IN ('subio', 'bajo')),
    activo BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT NOW()
);
```

### Variables necesarias:
```env
SUPABASE_URL=https://xxxxx.supabase.co
SUPABASE_API_KEY=eyJhbGc...
SUPABASE_PASSWORD=tu_database_password
SUPABASE_HOST=db.xxxxx.supabase.co
```

---

## 3. Token de SUNAT API

### Opción A: Obtener Token Oficial (Recomendado)

1. Visita la [página de SUNAT](https://e-consulta.sunat.gob.pe/)
2. Inspecciona las peticiones de red (F12 → Network)
3. Busca peticiones a `listarTipoCambio`
4. Copia el token del payload

### Opción B: Token de Prueba

Para desarrollo, puedes usar un token de prueba (puede expirar):
```
TOKEN_SUNAT_API=3eo8o3iyq8e6g8ygwu1kr5azkqpzfa4yk451kyl5aqyhdclc3u9u
```

⚠️ **Nota**: Este token es público y puede dejar de funcionar. Para producción, obtén tu propio token.

---

## 4. Variables de Entorno

Crea un archivo `.env` en la raíz del proyecto:

```env
# Database (para desarrollo local con Docker)
DB_USER=postgres
DB_PASSWORD=postgres
DB_HOST=localhost
DB_PORT=5432
DB_NAME=dolar_db

# Redis
REDIS_URL=redis://localhost:6379/0

# SUNAT API
TOKEN_SUNAT_API=tu_token_aqui

# Gmail
EMAIL_USER=tu_email@gmail.com
EMAIL_PASS=tu_app_password_sin_espacios
EMAIL_TO=destinatario@gmail.com

# Supabase
SUPABASE_URL=https://xxxxx.supabase.co
SUPABASE_API_KEY=eyJhbGc...
SUPABASE_PASSWORD=tu_database_password
SUPABASE_HOST=db.xxxxx.supabase.co
SUPABASE_PORT=5432
SUPABASE_DB=postgres

# WhatsApp (opcional - para futuras features)
PHONE_NUMBER_ID=
VERIFY_TOKEN=
WHATSAPP_TOKEN=
```

---

## 5. GitHub Actions Secrets

Para que el workflow automático funcione:

### Paso 1: Ir a Settings

1. En tu repositorio de GitHub
2. Ve a `Settings` → `Secrets and variables` → `Actions`
3. Click en `New repository secret`

### Paso 2: Agregar Secrets

Agrega cada uno de estos secrets:

| Nombre | Valor | Descripción |
|--------|-------|-------------|
| `TOKEN_SUNAT_API` | `3eo8o3i...` | Token de la API de SUNAT |
| `EMAIL_USER` | `tu@gmail.com` | Tu email de Gmail |
| `EMAIL_PASS` | `xxxxxxxxxxxx` | App Password (sin espacios) |
| `EMAIL_TO` | `destino@email.com` | Email que recibirá el reporte |
| `SUPABASE_URL` | `https://xxx.supabase.co` | URL de tu proyecto |
| `SUPABASE_API_KEY` | `eyJhbGc...` | API Key anon/public |

### Paso 3: Verificar

1. Ve a la pestaña `Actions`
2. Selecciona el workflow `Notificacion-dolar-diario`
3. Click en `Run workflow` para probar manualmente
4. Revisa los logs para verificar que todo funcione

---

## ✅ Verificación Final

### Checklist de Configuración

- [ ] Gmail App Password generado
- [ ] Proyecto Supabase creado
- [ ] Tabla `dolar` creada en Supabase
- [ ] Archivo `.env` configurado localmente
- [ ] Secrets configurados en GitHub
- [ ] Workflow ejecutado manualmente con éxito
- [ ] Email de prueba recibido

### Prueba Local

```bash
# Instalar dependencias
pip install -r requirements.txt

# Ejecutar ETL manualmente
python -m app.services.infrastructure.test_gmail

# Verificar que llegue el email
```

### Prueba con Docker

```bash
# Levantar servicios
docker-compose up -d

# Ver logs
docker-compose logs -f app

# Detener servicios
docker-compose down
```

---

## 🆘 Problemas Comunes

### "Authentication failed" en Gmail
- Verifica que uses App Password, no tu contraseña normal
- Asegúrate de que 2FA esté habilitado
- Elimina espacios del App Password

### "Invalid token" en SUNAT
- El token puede haber expirado
- Obtén un nuevo token siguiendo la Opción A

### "Connection refused" en Supabase
- Verifica que el proyecto esté activo (no pausado)
- Revisa que la URL y API Key sean correctas
- Verifica tu conexión a internet

### GitHub Action falla
- Revisa que todos los secrets estén configurados
- Verifica los logs en la pestaña Actions
- Asegúrate de que el repositorio sea público o tengas Actions habilitado

---

## 📞 Soporte

Si tienes problemas con la configuración:

1. Revisa los logs: `docker-compose logs app`
2. Verifica las variables de entorno
3. Abre un [issue en GitHub](https://github.com/jefersson1/dolar/issues)

---

**Última actualización**: Diciembre 2024
